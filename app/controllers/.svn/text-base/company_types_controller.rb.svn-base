class CompanyTypesController < ApplicationController
  ...
  def index
    @company_types = do_index(CompanyType, params)
  ...
  def create
    ...
    respond_to do |format|
      if @company_type.save
        format.html { redirect_to(@company_type, :notice => t("screens.notice.successfully_created")) }
  ...
  def update
    ...
    respond_to do |format|
      if @company_type.update_attributes(params[:company_type])
        format.html { redirect_to(@company_type, :notice => t("screens.notice.successfully_updated")) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    prudent_destroy(CompanyType.find(params[:id]))
  end
end
